...
export class MemberDetailScreen {
    ...
    readonly invalidMemberDescription;
     ...
     constructor(page: Page){
        ...
        this.kycBadge = page.locator('div.flex.items-center.gap-2', { hasText: 'KYC:' }).locator('span[data-slot="badge"]');
        this.cashoutBadge = page.locator('div.flex.items-center.gap-2', { hasText: 'Cashout:' }).locator('span[data-slot="badge"]');
        this.accountStatusBadge = page.locator('div.flex.items-center.gap-2', { hasText: 'Account:' }).locator('span[data-slot="badge"]');
    ...
    async verifyMemberNotFound(){
        ...
        await expect(this.invalidMemberDescription).toHaveText("The member you're looking for doesn't exist or has been removed.");
    }
    ...
    async verifyMemberNotReady(){
        ...
        await expect(this.invalidMemberDescription).toBeVisible();
        await expect(this.invalidMemberDescription).toHaveText("This member is pending review by account manager. Please wait.");
    }

    async mockMemberNotReadyPage(htmlFilePath?: string) {
        const filePath =
            htmlFilePath ||
            path.resolve(__dirname, '../../mocks/member-not-ready.html');
